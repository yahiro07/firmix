...
function collectKeyframeNames(slots: CssSlot[]): string[] {
  return uniqueArrayItems(
    slots
      .filter((slot) => slot.groupRuleSpec.startsWith("@keyframes"))
...
export function extractNestedCss(
  ...
  let cssText = [
    ...Object.values(slotsGroupedByConditionalGruopRule).map(stringifyCssSlots),
  ].join("\n");
  ...
  //calc()内での+が圧縮表記では機能しないので前後に空白を追加
  cssText = cssText.replace(new RegExp(/\+/, "g"), " + ");
